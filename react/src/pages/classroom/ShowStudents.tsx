...
export default function ShowStudents() {
  ...
  useEffect(() => {
    document.title = "Show Students | Classroom";
  ...
  const handleDelete = async (studentId: string) => {
    if (!user) {
      Swal.fire("Unauthorized!", "You are not allowed to delete students.", "error");
      return;
    }
            ...
            <tbody>
              {students.length > 0 ? (
                students.map((student, index) => (
                  <tr key={student.id} className={`border-b ${index % 2 === 0 ? "bg-gray-100" : "bg-gray-300"}`}>
